...
class GDBApp:
    ...
    def agregar_ficha_rph(self):
        """Importar el segundo archivo Excel y realizar la comparación con las feature classes r_lc_terreno_rph y u_lc_terreno_rph."""
        try:
            # Verificar si la geodatabase ha sido seleccionada antes de importar Excel
            gdb = self.gdb_path.get()
            if not gdb:
            ...
            if 'Npn' not in df_rph.columns or 'NroFicha' not in df_rph.columns:
                tkMessageBox.showerror("Error", "El archivo Excel no contiene las columnas 'Npn_rph' y 'NroFicha_rph'.")
                return
                    ...
                    with arcpy.da.UpdateCursor(fc_path, ['codigo_unidad_construccion', 'NroFicha']) as cursor:
                        for row in cursor:
                            ...
                            if codigo_unidad_construccion in npn_to_ficha_rph:
                                ...
                                cursor.updateRow(row)
                                print("Asignado NroFicha_rph a terreno_codigo"+ codigo_unidad_construccion+ "en "+ fc_name)

            tkMessageBox.showinfo("Éxito", "Datos del archivo Excel (RPH) importados y actualizados correctamente.")
